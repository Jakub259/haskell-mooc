...
sums i = sums' i i
  where
    sums' 0 _ = []
    sums' i n = sumInt (n - i + 1) : sums' (i -1) n
...
sorted :: [Int] -> Bool
sorted (x1 : x2 : xs) = (max x1 x2 == x2) && sorted (x2 : xs)
sorted xs = True
...
merge x [] = x
merge [] y = y
merge (x : xs) (y : ys) = if min x y == x then x : merge xs (y : ys) else y : merge (x : xs) ys

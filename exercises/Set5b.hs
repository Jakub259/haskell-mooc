...
isOrdered (Node x left right) = allValues (<x) left && isOrdered left && allValues (>x) right && isOrdered right
...
walk [StepL] (Node _ left _) =  valAtRoot left
walk [StepR] (Node _ _ right) = valAtRoot right
walk (x:xs) (Node val left right) = case x of 
    StepL -> walk xs left
    StepR -> walk xs right
...
set _ _ Empty = Empty
set [] val (Node x left right) = Node val left right
set (StepR : path) val (Node x left right) = Node x left (set path val right)
set (StepL : path) val (Node x left right) = Node x (set path val left) right
------------------------------------------------------------------------------

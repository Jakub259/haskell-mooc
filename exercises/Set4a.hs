...
comp :: (Eq a, Ord a) => [a] -> [a] -> Ordering
comp x y = case compare (length x) (length y) of 
    EQ -> compare (head y) (head x)
    ord -> ord
...
winner scores player1 player2 = case compare p1 p2 of 
    LT -> player2
    _ -> player1
    where p1 = Map.lookup player1 scores
          p2 = Map.lookup player2 scores

...
prettyPrint n qs = prettyPrint' n qs 1
  where
    coords = [(x, y) | x <- [1 .. n], y <- [1 .. n]]
    isQueen coord = coord `elem` qs
    printCoord coord = if isQueen coord then 'Q' else '.'
    prettyPrint' n qs x =
      if x <= n
        then map printCoord (filter (\coord -> fst coord == x) coords) ++ "\n" ++ prettyPrint' n qs (x + 1)
        else ""
...
danger c xs = getAny $ isDangerous c
  where
    predicates = [x y | x <- [sameRow, sameCol, sameDiag, sameAntidiag], y <- xs]
    isDangerous = foldMap (Any .) predicates
...
prettyPrint2 n qs = prettyPrint' n qs 1
  where
    coords = [(x, y) | x <- [1 .. n], y <- [1 .. n]]
    isQueen coord = coord `elem` qs
    printCoord coords
...
fixFirst n (x : xs)
  | snd x > n = Nothing
  | otherwise = if danger x xs then fixFirst' (nextCol x : xs) else Just (x : xs)
  where
    fixFirst' = fixFirst n
...
continue _ = error "oh no!"

backtrack :: Stack -> Stack
backtrack (_ : (x, y) : xs) = (x, y + 1) : xs
backtrack _ = error "oh no!"
...
finish n xs =
  if length xs > n then tail xs else finish n (step n xs)

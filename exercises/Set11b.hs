...
appendAll ioref xs = do
  forM_ xs (\x -> modifyIORef ioref (++ x))
...
hSelectLines h nums = do
  lines <- hFetchLines h
  let indexed = zip lines [1 ..]
   in return $ map fst $ filter (\line -> snd line `elem` nums) indexed
...
interact' f state = do
  line <- getLine
  let (cond, str, state') = f (line, state)
  putStrLn str
  if cond then interact' f state' else return state'

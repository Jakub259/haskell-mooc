...
data Country = Finland | Norway | Switzerland -- reordered to make default Ord work correctly | Ex.2
  deriving (Show,Eq,Ord) -- derirve Eq and Ord for Contry | Ex.1 Ex.2
...
instance Eq Name where
  Name name1 == Name name2 = let toLower' = map toLower
                             in toLower' name1 == toLower' name2
...
instance Eq a => Eq (List a) where
  ...
  Empty == _ = False
  _ == Empty = False
  LNode x xs == LNode y ys = (x==y) && (ys==xs)
...
instance Num RationalNumber where
  ...
  abs (RationalNumber qa qb) = RationalNumber (abs qa) (abs qb)
  signum (RationalNumber qa qb) = RationalNumber (signum qa) (signum qb)
  fromInteger x = RationalNumber x 1
  negate (RationalNumber qa qb) = RationalNumber (-qa) qb

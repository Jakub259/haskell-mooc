...
totalPrice (MkShoppingEntry _ d i) = fromIntegral i * d
...
buyOneMore (MkShoppingEntry str d i) = MkShoppingEntry str d (i+1)
------------------------------------------------------------------------------
...
setName name (Person name_before age) = Person name age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Person name age_before) = Person name age
...
rgb col = case col of
    ...
    Green -> [0,1,0]
    Blue -> [0,0,1]
    Invert color -> map (1 -) color'
        where color' = rgb color
    Mix color1 color2 -> [(r1 + r2)/2, (g1 + g2)/2, (b1 + b2)/2]
        where [r1,g1,b1] = rgb color1
              [r2,g2,b2] = rgb color2
...
toBin 0 = O End
toBin 1 = I End
toBin n = if odd n then 
      I (toBin (div n 2)) else 
      O (toBin (div n 2))

...
instance Functor TwoList where
  ...
  fmap f (TwoNode x1 x2 xs) = TwoNode (f x1) (f x2) (fmap f xs)
...
count elem = foldr (\x y -> if x == elem then y + 1 else y) 0
...
inBoth f1 f2 = foldr (\x y -> if x `elem` f2 then x : y else y) [] f1
...
instance Foldable List where
  foldr f d Empty = d
  foldr f d (LNode x xs) = f x (foldr f d xs)
...
instance Foldable TwoList where
  foldr f d TwoEmpty = d
  foldr f d (TwoNode x1 x2 xs) = f x1 (f x2 (foldr f d xs))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a l r) = sumTree l `mappend` a `mappend` sumTree r

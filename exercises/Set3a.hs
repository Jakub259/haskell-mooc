...
firstHalf str = take (uncurry (+) $ length str `divMod` 2) str
...
capitalize str = intercalate [' '] (map capitalizeFirst (words str))

capitalizeFirst :: String -> String
capitalizeFirst [] = []
capitalizeFirst (h : t) = toUpper h : t
...
joinToLength len str_list = [i ++ j | i <- str_list, j <- str_list, (length i + length j) == len]
...
interpreter commands = interpreter' commands 0 0 
    where interpreter' coms x y = case coms of [] -> []
                                               "up" : coms -> interpreter' coms x (y+1)
                                               "down" : coms -> interpreter' coms x (y-1)
                                               "right" : coms -> interpreter' coms (x+1) y
                                               "left" : coms -> interpreter' coms (x-1) y
                                               "printY": coms -> show y : interpreter' coms x y
                                               "printX": coms -> show x : interpreter' coms x y
                                               _ : coms -> interpreter' coms x y

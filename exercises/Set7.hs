...
member to_find (Set []) = False
member to_find (Set (x:xs)) =  x == to_find || member to_find (Set xs)

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add to_add (Set []) = Set [to_add]
add to_add (Set set) = Set (add' to_add set)
    where add' to_add [] = [to_add]
          add' to_add (x:xs) = case compare to_add x of
              LT -> to_add : x : xs
              EQ -> to_add : xs
              GT -> x : add' to_add xs
...
reverseNonEmpty (x :| xs ) =  head y :|  (tail y ++ [x])
    where y = reverse xs
------------------------------------------------------------------------------
...
passwordAllowed password (MinimumLength x) = length password >= x
passwordAllowed password (ContainsSome some) = foldr (\x y -> elem x password || y) False some
passwordAllowed password (DoesNotContain x) = not (passwordAllowed password (ContainsSome x))
passwordAllowed password (And x y) = passwordAllowed password x && passwordAllowed password y
passwordAllowed password (Or x y) = passwordAllowed password x || passwordAllowed password y
...
evaluate (Operation str x y) = let op "+" = (+)
                                   op "*" = (*)
                                   op _ = error "Bad operator"
                                   in op str (evaluate x) (evaluate y)


render :: Arithmetic -> String
render (Literal x) = show x
render (Operation str x y) = "(" ++ render x ++ str ++ render y ++ ")"
